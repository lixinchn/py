...
def getOldUser(page , update_time=""):
    ...
    sql = "select * from z_user where 1 %s order by ctime asc limit %s,%s"%(update_time,int(page*step),int(step))
    print sql
    m.Q(sql)
    rs = m.fetchall()
    m.close()
...
if __name__ == '__main__':
    ...
    date_arr = [(2014,9,30),(2014,10,31),(2014,11,30),(2014,12,31),
                (2015,1,31),(2015,2,28),(2015,3,31)]
    date_a = 6
    month = None
    if len(sys.argv) > 1:
        date_a = int(sys.argv[1])
    month = date_arr[date_a]
    #2014-09-05 00:00:00
    dateC=datetime.datetime(month[0],month[1],1,0,0,0)
    base_time =time.mktime(dateC.timetuple())
#     print timestamp
#     base_time = 1409846400.0
    dateC=datetime.datetime(month[0],month[1],month[2],0,0,1)
    end_time =time.mktime(dateC.timetuple())
    ltime=time.localtime(end_time)
    print "end time %s"%time.strftime("%Y-%m-%d", ltime)
    while(True):
        if base_time > end_time:
            break
        
        ltime=time.localtime(base_time)
        timeStr=time.strftime("%Y-%m-%d", ltime)
        print timeStr
        insertNewLine("and ctime >='%s 00:00:00' and ctime <= '%s 23:59:59' "%(timeStr,timeStr))
        base_time += 86400
